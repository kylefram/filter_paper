...
\end{table}
\newpage
We also include the full loss and $\Delta E_{00}$ results. 
...
Because the $k=8$ solution takes some time to brute force, we use a cosine similarity function to prune the filters to a set of 45, which reduces reconstruction quality and increases the differences in $\Delta E_{00}$ across renders. 
...
\begin{subfigure}{.49\linewidth}
    ...
    \caption{$k=8$, $\Delta E_{00}$=.61}
\end{subfigure}
\caption{CAVE Renders of Flowers Multispectral Dataset, With Macbeth $\Delta E_{00}$}
\end{figure}
...
We also demonstrate a classical camera tuning methodology, where we minimize explicitly the average $\Delta E_{00}$ over the PMC. 
For $k=3$, we achieve $\Delta E_{00}$ of 12, which is not good and should be investigated for errors in measurement. 
...
\begin{center}
    ...
    \captionof{figure}{$k=3$ reconstruction with $\Delta E_{00}$ minimization}
\end{center}